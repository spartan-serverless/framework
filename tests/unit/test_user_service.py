...
def mock_db_session():
    class MockSession:
        ...
        def filter(self, condition):
            self.query_condition = condition.right.value  # Assuming condition is User.id == id
            return self
        ...
        def filter(self, condition):
            ...
            self.filtered_users = [user for user in self.users if getattr(user, attribute) == value]
            return self
...
def test_save_user(mock_db_session):
    user_service = UserService(db=mock_db_session)
    user_request = UserCreateRequest(username="new_user", email="new_user@example.com", password="password")
...
def test_save_duplicate_email_user(mock_db_session):
    ...
    user_service = UserService(db=mock_db_session)
    user_request = UserCreateRequest(username="user2", email="user1@example.com", password="password")
...
def test_update_user(mock_db_session):
    add_users_to_session(mock_db_session, 1)
    user_service = UserService(db=mock_db_session)
    user_request = UserUpdateRequest(username="updated_user", email="updated_user@example.com")
